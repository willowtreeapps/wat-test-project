 ...
 *
 * - `full` Ð `{string}` Ð Full version string, such as "0.9.18".
 * - `major` Ð `{number}` Ð Major version number, such as "0".
 * - `minor` Ð `{number}` Ð Minor version number, such as "9".
 * - `dot` Ð `{number}` Ð Dot version number, such as "18".
 * - `codeName` Ð `{string}` Ð Code name of the release, such as "jiggling-armfat".
 */
var version = {
 ...
 *
 *   - `{number=}` `capacity` Ñ turns the cache into LRU cache.
 *
 * @returns {object} Newly created cache object with the following set of methods:
 *
 * - `{object}` `info()` Ñ Returns id, size, and options of cache.
 * - `{void}` `put({string} key, {*} value)` Ñ Puts a new key-value pair into the cache.
 * - `{{*}} `get({string} key) Ñ Returns cached value for `key` or undefined for cache miss.
 * - `{void}` `remove({string} key) Ñ Removes a key-value pair from the cache.
 * - `{void}` `removeAll() Ñ Removes all cached values.
 * - `{void}` `destroy() Ñ Removes references to this cache from $cacheFactory.
 *
 */
 ...
 * **Methods**
 *
 * - `resolve(value)` Ð resolves the derived promise with the `value`. If the value is a rejection
 *   constructed via `$q.reject`, the promise will be rejected instead.
 * - `reject(reason)` Ð rejects the derived promise with the `reason`. This is equivalent to
 *   resolving it with a rejection constructed via `$q.reject`.
 *
 * **Properties**
 *
 * - promise Ð `{Promise}` Ð promise object associated with this deferred.
 *
 *
 * # The Promise API
 *
 ...
 * **Methods**
 *
 * - `then(successCallback, errorCallback)` Ð regardless of when the promise was or will be resolved
...
function $RouteProvider(){
   ...
   *    Object properties:
   *
   *    - `controller` Ð `{function()=}` Ð Controller fn that should be associated with newly
   *      created scope.
   *    - `template` Ð `{string=}` Ð path to an html template that should be used by
   ...
   *    - `redirectTo` Ð {(string|function())=} Ð value to update
...
function $HttpProvider() {
  ...
  this.$get = ['$httpBackend', '$browser', '$cacheFactory', '$rootScope', '$q', '$injector',
     ...
     *
     * # General usage
     * The `$http` service is a function which takes a single argument Ñ a configuration object Ñ
     ...
     * the `then` method to register callbacks, and these callbacks will receive a single argument Ð
     ...
     * injected with dependencies (if specified) and returns the interceptor  Ñ a function that
     ...
     *
     *    - **method** Ð `{string}` Ð HTTP method (e.g. 'GET', 'POST', etc)
     *    - **url** Ð `{string}` Ð Absolute or relative URL of the resource that is being requested.
     *    - **params** Ð `{Object.<string|Object>}` Ð Map of strings or objects which will be turned to
     *      `?key1=value1&key2=value2` after the url. If the value is not a string, it will be JSONified.
     *    - **data** Ð `{string|Object}` Ð Data to be sent as the request message data.
     *    - **headers** Ð `{Object}` Ð Map of strings representing HTTP headers to send to the server.
     *    - **transformRequest** Ð `{function(data, headersGetter)|Array.<function(data, headersGetter)>}` Ð
     *      transform function or an array of such functions. The transform function takes the http
     *      request body and headers and returns its transformed (typically serialized) version.
     *    - **transformResponse** Ð `{function(data, headersGetter)|Array.<function(data, headersGetter)>}` Ð
     *      transform function or an array of such functions. The transform function takes the http
     *      response body and headers and returns its transformed (typically deserialized) version.
     *    - **cache** Ð `{boolean|Cache}` Ð If true, a default $http cache will be used to cache the
     *      GET request, otherwise if a cache instance built with
     *      {@link angular.module.ng.$cacheFactory $cacheFactory}, this cache will be used for
     *      caching.
     *    - **timeout** Ð `{number}` Ð timeout in milliseconds.
     ...
     *
     *   - **data** Ð `{string|Object}` Ð The response body transformed with the transform functions.
     *   - **status** Ð `{number}` Ð HTTP status code of the response.
     *   - **headers** Ð `{function([headerName])}` Ð Header getter function.
     *   - **config** Ð `{Object}` Ð The configuration object that was used to generate the request.
     *
 ...
 * only public api is:
 *
 * * `id` Ð `{string}` Ð locale id formatted as `languageId-countryId` (e.g. `en-us`)
 */
 ...
 * @example
   <doc:example>
     <doc:source>
       ...
       Phone only <input ng-model="search.phone"Œ><br>
 ...
 * @returns {string} Number rounded to decimalPlaces and places a Ò,Ó after each third digit.
 *
 * @example
   <doc:example>
     <doc:source>
 ...
 *  forms, where:
 *
 *  - keys are validation tokens (error names) Ñ such as `REQUIRED`, `URL` or `EMAIL`),
 ...
 *
 * * Model Ñ The Model is data in scope properties; scopes are attached to the DOM.
 * * View Ñ The template (HTML with data bindings) is rendered into the View.
 * * Controller Ñ The `ngController` directive specifies a Controller class; the class has
 ...
 *   * `$position` Ð `{string}` Ð position of the repeated element in the iterator. One of:
 *        * `'first'`,
 *        * `'middle'`
 *        * `'last'`
 *
